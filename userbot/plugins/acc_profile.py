...
from uniborg.util import sycrox_cmd
from userbot.Config import Var


@borg.on(sycrox_cmd(pattern="pbio (.*)"))  # pylint:disable=E0602
async def _(event):
    if event.fwd_from:
        return
...
@borg.on(sycrox_cmd(pattern="pname ((.|\n)*)"))  # pylint:disable=E0602,W0703
async def _(event):
    if event.fwd_from:
        return
...
@borg.on(sycrox_cmd(pattern="ppic"))  # pylint:disable=E0602
async def _(event):
    if event.fwd_from:
        return
    ...
    try:
        photo = await borg.download_media(  # pylint:disable=E0602
            reply_message, Var.TEMP_DOWNLOAD_DIRECTORY  # pylint:disable=E0602
        )
    except Exception as e:  # pylint:disable=C0103,W0703
        await event.edit(str(e))
    else:
