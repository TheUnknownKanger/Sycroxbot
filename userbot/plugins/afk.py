    ...
    if ".afk" not in current_message and "yes" in USER_AFK:  # pylint:disable=E0602
        ...
        try:
            ...
            await borg.send_message(  # pylint:disable=E0602
                ...
                + "Ask In @sycroxsupport Chat grp to get help..\n\n `{}`".format(
                    str(e)
                ),
...
async def on_afk(event):
    ...
    if USER_AFK and not (await event.get_sender()).bot:  # pylint:disable=E0602
        ...
        msg = None
        message_to_reply = (
            f"Hey!! My master is currently offline... Since when?\n**For** `{total_afk_time}`\nLEL Btw He Is Offline For Some Important Reason He Is very Busy "
...
@borg.on(sycrox_cmd(pattern=r"afk ?(.*)", outgoing=True))  # pylint:disable=E0602
async def _(event):
    if event.fwd_from:
        return
    global USER_AFK  # pylint:disable=E0602
